...
class Block:
    ...
    def __init__(self,previous_hash, amount, signature, username, certificate, first_symmetric_key, second_symmetric_key):
        ...
        self.hash = Block.build_hash(previous_hash, amount, signature, username, certificate)
        self.timestamp = datetime.datetime.now()


    @classmethod
    def build_hash(cls, previous_hash, amount, signature, username, certificate):
        digest = hashes.Hash(hashes.SHA256(), backend=default_backend())
        data = str(previous_hash)+str(amount)+str(signature)+str(username)+str(certificate)
